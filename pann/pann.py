...
class CrossValidation:
    ...
    def load_raw_data(self,kind_data:str):
        raw_data = self.transformation.load_raw_data(kind_data=kind_data)
        return raw_data
    ...
    def load_scaler (self,split, test_raw_data:TensorDataset, clamp = False):
    ...
    def load_model (self,drop:float):
        #load model
        model = self.transformation.load_model(drop=drop,device=self.device)    
        return model
    ...
    def train_evaluate_loop(self,run:neptune.init_run,model:nn.Module,train_loader,val_loader,epochs,optimizer:torch.optim,cv:bool=True):
        ...
        for iter in range(epochs):
            ...
            for X_train,y_train in train_loader:
                ...
                #load data to device
                X_hor_train = X_train[:,0].float().to(self.device)
                X_ver_train = X_train[:,1].float().to(self.device)
                ...
                #print("Weight Gradients:", model.fc1.weight.grad.ravel()[0:10])
                #print("Weight Gradients:", model.fc2.weight.grad.ravel()[0:10])
            ...
            with torch.inference_mode():
                for X_val,y_val in val_loader:
                    ...
                    X_hor_val = X_val[:,0].float().to(self.device)
                    X_ver_val = X_val[:,1].float().to(self.device)
                    y_val = y_val[:,0].to(self.device)
                    
                    #forward pass
                    y_val_pred = model(X_hor_val,X_ver_val).ravel()
    ...
    def cv_load_data_parallel (self, batch_size, params_idx):
        ...
        train_raw_data, val_raw_data, scaler_raw = self.normalize_raw(train_val_raw_data=self.train_val_raw_data,train_index=train_idx,val_index=val_idx,min_max_scaler=False)        
        
        #turn data to data loader
        train_loader = self.data_loader(data=train_raw_data,batch_size=batch_size)   
        val_loader = self.data_loader(data=val_raw_data,batch_size=batch_size)
        
        #save the scaler:
        self.save_scaler(split=split,scaler_raw = scaler_raw)
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(batch_size,params_idx) for params_idx in enumerate(kf.split(self.train_val_raw_data)))
    ...
    def cv_eval_final_loop (self,model:torch.nn,full_raw_data:TensorDataset,batch_size,params):
        #function to transform test_raw_data with the saved scaler from 
        split, file = params
        
        #create test_loader
        test_data = self.load_scaler(split=split,test_raw_data=full_raw_data)
        ...
        with torch.inference_mode():
            for X_test,y_test in test_loader:
                ...
                X_hor_test = X_test[:,0].float().to(self.device)
                X_ver_test = X_test[:,1].float().to(self.device)
                y_test = y_test[:,0].to(self.device)
                
                #forward pass
                y_test_pred = model(X_hor_test,X_ver_test).ravel()
    ...
    def best_cv_final (self, run:neptune.init_run,model:torch.nn ,best_trial_number:int,full_raw_data:TensorDataset,batch_size):
        #function to calculate the result of the 6 best model cv
        ...
        result_eval_best_cv = Parallel(n_jobs=N_JOBS_BEST_CV)(delayed(self.cv_eval_final_loop)(model,full_raw_data,batch_size,params) for params in enumerate(pre_trained_models))
    ...
    def final_test (self,best_params:dict,best_trial_number):
        ...
        #load data
        full_data = self.load_raw_data(kind_data="full")
...
def objective(trial:optuna.trial.Trial):
    ...
    #cross validation
    loss_val = cross.cross_validation(trial=trial,drop=drop,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay)
    
    return loss_val

#project 
PROJECT = "ba-final/pann-2"
...
EPOCHS = [100]
n_trials = 1
fix_parameters = {"k":K,"seed":SEED,"w_size":W_SIZE,"n_trials":TOTAL_TRIALS,"batch_size":BATCH_SIZE,"epochs":EPOCHS}

#interval 
DROP = (0.1,0.8,True)
OPTIMIZER = ("Adam","SGD")
LR = (0.000001,0.01,True)
WEIGHT_DECAY = (0.000001,0.2,True)
fix_interval = {"optmizer":OPTIMIZER,"learning_rata":LR,"batch_size":BATCH_SIZE,"epochs":EPOCHS}
