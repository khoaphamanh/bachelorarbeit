...
class DataPreprocessing:
    ...
    def norm(self,bearing:int):
        name_file = "norm_{}.npy".format(bearing)
        file_acc = os.path.join(self.data_dir, name_file)
        norm_data = np.load(file_acc)
        return norm_data.T

    def stand(self,bearing:int):
        name_file = "stand_{}.npy".format(bearing)
        file_acc = os.path.join(self.data_dir, name_file)
        center_data = np.load(file_acc)
        return center_data.T
