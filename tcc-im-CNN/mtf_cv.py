...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data, test_raw_data, n_bins, strategy):
        ...
        train_image = self.transformation.mtf(train_raw_data, n_bins, strategy)
        test_image =  self.transformation.mtf(test_raw_data, n_bins, strategy)
    ...
    def cv_load_data_parallel (self, n_bins, strategy,val_bearing):
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float, embedding_dim :int, n_bins, strategy,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float,n_FPT:int, n_period:int):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(n_bins, strategy,val_bearing) for val_bearing in enumerate(self.bearing_train_number))
...
def objective(trial:optuna.trial.Trial):
    ...
    #cross validation
    score_split = cross.cross_validation(trial=trial,drop=drop,n_bins=n_bins,strategy=strategy,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay,embedding_dim=embedding_dim,n_FPT=n_FPT,n_period=n_period)
...
fix_interval = {"n_bins":N_BINS,"strategy":STRATEGY,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"embedding_dim":EMBEDDING_DIM, "n_FPT":N_FPT,"n_period":N_PERIOD}
