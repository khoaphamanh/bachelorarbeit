...
class CrossValidation:
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float, embedding_dim :int, b , fc , scale_min, scale_max, n_scales, optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float,n_FPT:int, n_period:int):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(b, fc ,scale_min, scale_max, n_scales, val_bearing) for val_bearing in enumerate(self.bearing_train_number))
...
fix_interval = {"b":B,"fc":FC,"scale_min":SCALE_MIN,"scale_max":SCALE_MAX,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"embedding_dim":EMBEDDING_DIM, "n_FPT":N_FPT,"n_period":N_PERIOD}
