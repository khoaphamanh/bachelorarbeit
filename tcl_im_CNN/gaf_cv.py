...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data, test_raw_data, method:str):
        #finction to transform raw_norm_data to image and normalize pixel it
        train_image = self.transformation.gaf(train_raw_data,method=method) 
        test_image =  self.transformation.gaf(test_raw_data,method=method)
    ...
    def cv_load_data_parallel (self,method:str,val_bearing):
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float, embedding_dim :int,method:str,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float,n_FPT:int, n_period:int):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(method,val_bearing) for val_bearing in enumerate(self.bearing_train_number))
...
def objective(trial:optuna.trial.Trial):
    ...
    #cross validation
    score_split = cross.cross_validation(trial=trial,drop=drop,method = method,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay,embedding_dim=embedding_dim,n_FPT=n_FPT,n_period=n_period)
...
fix_interval = {"method":METHODD,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"embedding_dim":EMBEDDING_DIM, "n_FPT":N_FPT,"n_period":N_PERIOD}
