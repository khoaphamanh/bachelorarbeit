...
class CrossValidation:
    ...
    def cv_load_data_parallel (self,w_stft:int, hop:int,n_mels:int,val_bearing):
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float, embedding_dim :int,w_stft:int, hop:int,n_mels:int,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float,n_FPT:int, n_period:int):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(w_stft,hop,n_mels,val_bearing) for val_bearing in enumerate(self.bearing_train_number))
...
def objective(trial:optuna.trial.Trial):
    ...
    #cross validation
    score_split = cross.cross_validation(trial=trial,drop=drop,w_stft=w_stft,hop=hop,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay,embedding_dim=embedding_dim,n_FPT=n_FPT,n_period=n_period,n_mels = n_mels)
...
fix_interval = {"w_stft":W_STFT,"hop":HOP,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"embedding_dim":EMBEDDING_DIM, "n_FPT":N_FPT,"n_period":N_PERIOD,"n_mels":N_MELS}
