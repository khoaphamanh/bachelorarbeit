from helper_tcc import DataPreprocessing
import numpy as np
import os
import torch
...
def creat_dataset(kind_data:str,window_size:int):
    ...
    path_dir = os.path.join(cwd,"data_tcc".format(window_size))    
    ...
    if os.path.isfile(path_file) == False:
        ...
        for index_train, bearing in enumerate (bearing_number):
            ...
            X_train_bearing = np.zeros(shape = (n_windows_bearing,2,window_size))
            y_train_bearing = np.zeros(shape = (n_windows_bearing,2))
            ...
            for index,label in enumerate(range(min_range,max_range,100)):
                #scale label
