...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data, test_raw_data, w_stft, hop, n_mels):
        #finction to transform raw_norm_data to image and normalize pixel it
        train_image = self.transformation.lms(train_raw_data,w_stft=w_stft,hop=hop,n_mels=n_mels) 
        test_image =  self.transformation.lms(test_raw_data,w_stft=w_stft,hop=hop,n_mels=n_mels) 
    ...
    def load_scaler (self,split, test_raw_data:TensorDataset,w_stft, hop,n_mels, clamp = False):
        ...
        #transform to image
        test_data_image = self.transformation.lms(test_data,w_stft,hop,n_mels)  
    ...
    def cv_load_data_parallel (self,batch_size,w_stft:int, hop:int,n_mels:int,params_idx):
        ...
        train_data, val_data, scaler_pixel = self.transform_image(train_raw_data=train_raw_data,test_raw_data=val_raw_data,w_stft=w_stft,hop=hop,n_mels = n_mels)          
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float,w_stft:int, hop:int,n_mels:int,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(batch_size,w_stft,hop,n_mels,params_idx) for params_idx in enumerate(kf.split(self.train_val_raw_data)))
    ...
    def final_test (self,best_params:dict,best_trial_number):
        ...
        train_data, full_data, _ = self.transform_image(train_raw_data=train_raw_data,test_raw_data=full_raw_data,w_stft=w_stft,hop=hop,n_mels=n_mels)
...
def objective(trial:optuna.trial.Trial):
    ...
    w_stft = trial.suggest_int(name="w_stft",low=W_STFT[0],high=W_STFT[1],step=W_STFT[2])
    hop = trial.suggest_int(name="hop",low=HOP[0],high=HOP[1],step=HOP[2])
    n_mels = trial.suggest_int(name="n_mels",low=N_MELS[0],high=N_MELS[1],step=N_MELS[2])
    ...
    #cross validation
    loss_val = cross.cross_validation(trial=trial,drop=drop,w_stft=w_stft,hop=hop,n_mels=n_mels,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay)
    
    return loss_val

#project 
PROJECT = "ba-final/lms-2"
...
fix_interval = {"w_stft":W_STFT,"hop":HOP,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"n_mels": N_MELS}
