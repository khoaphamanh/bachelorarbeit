...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data, test_raw_data, w_stft, hop):
        #finction to transform raw_norm_data to image and normalize pixel it
        train_image = self.transformation.stft(train_raw_data,w_stft=w_stft,hop=hop) 
        test_image =  self.transformation.stft(test_raw_data,w_stft=w_stft,hop=hop)
    ...
    def load_scaler (self,split, test_raw_data:TensorDataset,w_stft, hop, clamp = False):
        ...
        #transform to image
        test_data_image = self.transformation.stft(test_data,w_stft,hop)    
    ...
    def cv_load_data_parallel (self,batch_size,w_stft:int, hop:int,params_idx):
        ...
        train_data, val_data, scaler_pixel = self.transform_image(train_raw_data=train_raw_data,test_raw_data=val_raw_data,w_stft=w_stft,hop=hop)          
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float,w_stft:int, hop:int,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(batch_size,w_stft,hop,params_idx) for params_idx in enumerate(kf.split(self.train_val_raw_data)))
    ...
    def final_test (self,best_params:dict,best_trial_number, evaluate = False):
        ...
        train_data, full_data, _ = self.transform_image(train_raw_data=train_raw_data,test_raw_data=full_raw_data,w_stft=w_stft,hop=hop)
...
def objective(trial:optuna.trial.Trial):
    ...
    #cross validation
    loss_val = cross.cross_validation(trial=trial,drop=drop,w_stft=w_stft,hop=hop,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay)
    
    return loss_val

#project 
PROJECT = "ba-final/test-1"
...
fix_interval = {"w_stft":W_STFT,"hop":HOP,"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY}
