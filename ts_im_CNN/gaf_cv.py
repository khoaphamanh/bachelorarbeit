...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data, test_raw_data, method):
        #finction to transform raw_norm_data to image and normalize pixel it
        train_image = self.transformation.gaf(train_raw_data,method) 
        test_image =  self.transformation.gaf(test_raw_data,method) 
    ...
    def load_scaler (self,split, test_raw_data:TensorDataset, method, clamp = False):
    ...
    def cv_load_data_parallel (self,batch_size,method:str,params_idx):
        ...
        train_data, val_data, scaler_pixel = self.transform_image(train_raw_data=train_raw_data,test_raw_data=val_raw_data,method = method)
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float, method:str,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(batch_size,method,params_idx) for params_idx in enumerate(kf.split(self.train_val_raw_data)))
    ...
    def best_cv_final (self, run:neptune.init_run,model:torch.nn ,best_trial_number:int,full_raw_data:TensorDataset,batch_size, method):
        ...
        #load data parallel:
        test_data = Parallel(n_jobs=N_JOBS_LOAD_DATA_BEST_CV)(delayed(self.load_scaler)(split=split,test_raw_data=full_raw_data,method=method,clamp= CLAMP) for split in range(0,self.k))
    ...
    def final_test (self,best_params:dict,best_trial_number):
        ...
        train_data, full_data, _ = self.transform_image(train_raw_data=train_raw_data,test_raw_data=full_raw_data,min_max_scaler=MIN_MAX_SCALER,clamp=CLAMP)
        ...
        #final best 6 cv
        text = self.best_cv_final(run=run,model=model,best_trial_number=best_trial_number,full_raw_data=full_data_raw,batch_size=batch_size,method = method)
        print(text)
...
def objective(trial:optuna.trial.Trial):
    ...
    #load hyperparameter
    method = trial.suggest_categorical(name="method",choices=METHODD)
    ...
    #cross validation
    loss_val = cross.cross_validation(trial=trial,drop=drop,method=method,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay)
    
    return loss_val

#project 
PROJECT = "ba-final/gaf-2"
...
#interval 
METHODD= ("summation", "difference")

DROP = (0.1,0.8,True)
OPTIMIZER = ("Adam","SGD")
LR = (0.0005,0.2,True)
WEIGHT_DECAY = (0.00001,0.1,True)
fix_interval = {"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"method":METHODD}
