...
class CrossValidation:
    ...
    def transform_image (self, train_raw_data,test_raw_data, b, fc, scale_min, scale_max, n_scales):
        #finction to transform raw_norm_data to image and normalize pixel it
        train_image = self.transformation.cwt(train_raw_data, b = b, fc = fc, scale_min=scale_min, scale_max=scale_max, n_scales=n_scales) 
        test_image =  self.transformation.cwt(test_raw_data, b = b, fc = fc, scale_min=scale_min, scale_max=scale_max, n_scales=n_scales) 
    ...
    def load_scaler (self,split, test_raw_data:TensorDataset, b, fc, scale_min, scale_max, n_scales, clamp = False):
        ...
        #transform to image
        test_data_image = self.transformation.cwt(test_data, b = b, fc = fc, scale_min=scale_min, scale_max=scale_max, n_scales=n_scales)   
    ...
    def cv_load_data_parallel (self,batch_size,b:float,fc:float, scale_min:float, scale_max:float, n_scales:int,params_idx):
        ...
        train_data, val_data, scaler_pixel = self.transform_image(train_raw_data=train_raw_data,test_raw_data=val_raw_data,b = b, fc = fc, scale_min = scale_min, scale_max = scale_max, n_scales = n_scales)                    
    ...
    def cross_validation(self,trial:optuna.trial.Trial,drop:float,b:float,fc:float, scale_min:float, scale_max:float, n_scales:int,optimizer_name:str,lr:float,batch_size:int,epochs:int,weight_decay:float):
        ...
        load_data = Parallel(n_jobs=N_JOBS_LOAD_DATA)(delayed(self.cv_load_data_parallel)(batch_size,b,fc, scale_min, scale_max, n_scales,params_idx) for params_idx in enumerate(kf.split(self.train_val_raw_data)))
    ...
    def best_cv_final (self, run:neptune.init_run,model:torch.nn ,best_trial_number:int,full_raw_data:TensorDataset,batch_size, b , fc , scale_min, scale_max,n_scales): 
        ...
        #load data parallel:
        test_data = Parallel(n_jobs=N_JOBS_LOAD_DATA_BEST_CV)(delayed(self.load_scaler)(split=split,test_raw_data=full_raw_data,b = b, fc = fc, scale_min=scale_min, scale_max = scale_max,n_scales = n_scales, clamp= CLAMP) for split in range(0,self.k))
    ...
    def final_test (self,best_params:dict,best_trial_number, evaluate = False):
        ...
        #load hyperparamters
        b, fc , scale_min, scale_max, n_scales  = best_params["b"], best_params["fc"], best_params["scale_min"], best_params["scale_max"], best_params["n_scales"]
        ...
        train_data, full_data, _ = self.transform_image(train_raw_data=train_raw_data,test_raw_data=full_raw_data,b = b, fc = fc, scale_min=scale_min, scale_max=scale_max,n_scales=n_scales) 
        ...
        text = self.best_cv_final(run=run,model=model,best_trial_number=best_trial_number,full_raw_data=full_data_raw,b = b, fc = fc, scale_min=scale_min, scale_max=scale_max,n_scales=n_scales,batch_size = batch_size) 
        print(text)
...
def objective(trial:optuna.trial.Trial):
    ...
    b = trial.suggest_float(name="b",low=B[0],high=B[1],step=B[2])
    fc = trial.suggest_float(name="fc",low=FC[0],high=FC[1],step=FC[2])
    scale_min = trial.suggest_float(name="scale_min",low=SCALE_MIN[0],high=SCALE_MIN[1],step=SCALE_MIN[2])
    scale_max = trial.suggest_int(name="scale_max",low=SCALE_MAX[0],high=SCALE_MAX[1],step=SCALE_MAX[2])
    n_scales = trial.suggest_int(name="n_scales",low=N_SCALES[0],high=N_SCALES[1],step=N_SCALES[2])
    ...
    #cross validation
    loss_val = cross.cross_validation(trial=trial,drop=drop,b = b,fc = fc,scale_min = scale_min, scale_max = scale_max, n_scales = n_scales,optimizer_name=optimizer_name,lr=lr,batch_size=batch_size,epochs=epochs,weight_decay=weight_decay)
    
    return loss_val

#project 
PROJECT = "ba-final/cwt-2"
...
fix_interval = {"optmizer":OPTIMIZER,"learning_rata":LR,"weight_decay":WEIGHT_DECAY,"b":B,"fc":FC,"scale_min":SCALE_MIN,"scale_max":SCALE_MAX}
